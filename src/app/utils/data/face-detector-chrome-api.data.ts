import { IProject } from '../IProject';

export const FaceDetectionData: IProject = {
  id: 'A1B3308D-8BC5-4A7B-81A7-4190A9A41D0B',
  name: 'Face Detection 🔎',
  description: 'Face Detection using Google Chrome API',
  projectLink: 'https://github.com/Amita Singh/face-detector-chrome-api',
  liveUrl: 'https://Amita Singh.github.io/face-detector-chrome-api',
  features: ['Face Detection api of Google Chrome'],
  tags: ['others', 'chrome'],
  techStuff: ['Chrome API'],
  additionalData: [
    {
      title: 'Note',
      data: ['This is an experimental features of Google Chrome']
    }
  ]
};
